...
public class frmXemDiem extends javax.swing.JFrame {
   ...
   private DefaultTableModel dtm;
    ...
    private void loaddata() {
        try {
             ListDiem =new DiemDAO().findByMaSV(Login.nameLogin);
            
           
            
            for (Diem diem : ListDiem ) {
                dtm.addRow(toObjectsData(diem));
            }
            jtbDSLopHocPhan.setModel(dtm);
        } catch (InstantiationException ex) {
            Logger.getLogger(frmXemDiem.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public static Object[] toObjectsData(Diem diem) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        MonHoc mh =new MonHoc();
        IMonHocDAO monHocDAO = (IMonHocDAO) Class.forName("BLL.MonHocDAO").newInstance();
        ArrayList<MonHoc> listMH = new MonHocDAO().findIDMaMH(diem.getMaMH());
        Object[] objectsData = {diem.getMaMH(),listMH.get(0).getTenMH(), diem.getDiemTK(),diem.getDiemGK(),diem.getDiemCK(),diem.getDiemTB(),diem.getXepLoai(),listMH.get(0).getHocKy()};
        return objectsData;
    }
    ...
    private void initComponents() {
        ...
        jtbDSLopHocPhan.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã Môn Học", "Tên Môn Học", "Điểm Thường Kỳ", "Điểm Giữa Kỳ", "Điểm Cuối Kỳ", "Điểm Trung Bình", "Xếp Loại", "Học Kỳ", "Năm Học"
            }
        ) {
